...
app.get("/api/admin/stats", async (req, res) => {
  try {
    ...
    const totalRevenue = revenueData
      ? revenueData.reduce((sum, b) => sum + parseFloat(b.total_amount), 0)
      : 0;
    res.json({
      success: true,
      stats: {
...
app.post(
  ...
  async (req, res) => {
    try {
      const { name, type, capacity, price_per_night, description, amenities } =
        req.body;
  ...
  async (req, res) => {
    try {
      ...
      if (error)
        return res
          .status(500)
          .json({ success: false, error: "Failed to update room." });
      res.json({ success: true, room: data });
    } catch (error) {
      res.status(500).json({
        success: false,
  ...
  async (req, res) => {
    try {
      ...
      if (error)
        return res
          .status(500)
          .json({ success: false, error: "Failed to delete room." });
...
app.put("/api/businesses/:id", authenticateBusiness, async (req, res) => {
  try {
    const { cover_image, logo_url, about_text, description, website, name } =
      req.body;
    const updateData = {};
...
app.post("/api/bookings", async (req, res) => {
  try {
    ...
    if ((bookingCount || 0) >= (businessLimit?.booking_limit || 50))
      return res
        .status(403)
        .json({ success: false, error: "Free booking limit reached." });
    let bookingData = {
...
app.delete("/api/staff/:staffId", authenticateBusiness, async (req, res) => {
  try {
    await supabase.from("staff").delete().eq("id", req.params.staffId);
