...
function HomePage() {
    ...
    const checkCustomDomain = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE}/api/domain-info?domain=${currentDomain}`,
        );
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.businesses.length === 0)
          showError(
            `No ${selectedCategory} found in ${location || "that area"}`,
          );
      }
    } catch (error) {
  ...
  if (showHostLanding)
    return <HostLanding onBack={() => setShowHostLanding(false)} />;
  if (businessUser) {
  ...
  const getCategoryPlaceholder = () => {
    if (selectedCategory === "hotel") return "e.g., Abuja, Lagos";
    if (selectedCategory === "sports") return "e.g., Abuja, Lagos";
    if (selectedCategory === "event") return "e.g., Abuja, Lagos";
  };
    ...
    <div className="app-container">
      ...
      <div className="app-header page-enter">
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          <button
            className="btn btn-secondary refined-focus"
            onClick={goToAdmin}
          >
          ...
          <button
            className="btn btn-secondary refined-focus"
            onClick={() => setShowHostLanding(true)}
          >
            Become a Host
          </button>
          <button
            className="btn btn-primary refined-focus"
      ...
      >
        <button
          className={`btn ${selectedCategory === "hotel" ? "btn-primary" : "btn-secondary"}`}
        ...
        >
          <Building2 size={20} strokeWidth={2} /> Hotels
        </button>
        <button
          className={`btn ${selectedCategory === "sports" ? "btn-primary" : "btn-secondary"}`}
        ...
        >
          <Trophy size={20} strokeWidth={2} /> Sports
        </button>
        <button
          className={`btn ${selectedCategory === "event" ? "btn-primary" : "btn-secondary"}`}
        ...
        >
          <PartyPopper size={20} strokeWidth={2} /> Events
        </button>
      </div>
      ...
      <div
        ...
        style={{
          ...
          transform: heroTransitioning ? "scale(1.02)" : "scale(1)",
      ...
      >
        ...
        >
          ...
          >
            {heroDescriptions[selectedCategory]}
          </p>
        </div>
      </div>
      ...
      >
        <span>13 venues across Lagos and Abuja</span>
        <span>.</span>
        <span>Instant confirmation</span>
        <span>.</span>
        <span>Pay online or at venue</span>
      </div>
      ...
      <div className="search-card page-enter stagger-4">
        <div className="form-group">
          ...
          <input
            ...
            className="form-control refined-focus"
          />
        </div>
        ...
        >
          ...
          {loading
            ? "Searching..."
            : `Search ${selectedCategory === "hotel" ? "Hotels" : selectedCategory === "sports" ? "Sports Facilities" : "Event Venues"}`}
        </button>
      </div>
        ...
        <div className="hotels-grid">
          ...
          {[1, 2, 3].map((i) => (
            ...
            >
              <div
                className="skeleton"
                style={{ height: "140px", borderRadius: "12px 12px 0 0" }}
              ></div>
      ...
      {!loading && results.length > 0 && (
        <div className="hotels-grid">
          ...
          {results.map((business, index) => (
            <div
              key={business.id}
              className={`hotel-card premium-card page-enter stagger-${Math.min(index + 1, 6)}`}
            ...
            >
              <div
                style={{
                  width: "100%",
                  height: "140px",
                  backgroundImage: `url(${getBusinessImage(business.business_type, index)})`,
                  backgroundSize: "cover",
                  backgroundPosition: "center",
                  borderRadius: "12px",
                  marginBottom: "16px",
              ...
              <div className="hotel-location">
                <MapPin size={14} strokeWidth={2} /> {business.address},{" "}
                {business.city}
              </div>
              ...
              <div style={{ display: "flex", gap: "8px", marginTop: "16px" }}>
                {business.business_type === "hotel" && (
                  <button
                    ...
                    className="btn btn-success refined-focus"
                    style={{ flex: 1 }}
                  >
                    View Rooms
                  </button>
      ...
      {!loading && results.length === 0 && location && (
        <div className="empty-state page-enter">
          <div className="empty-state-icon">
            <Search size={64} strokeWidth={1.5} color="var(--gray-400)" />
          </div>
