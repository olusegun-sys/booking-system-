...
function BusinessSettings({ business, onBack, onBusinessUpdate }) {
  ...
  function fetchData() {
    setLoading(true);
    Promise.all([fetchOperatingHours(), fetchBlockedDates()]).finally(
      function () {
        setLoading(false);
      },
    );
  ...
  function fetchOperatingHours() {
    return fetch(
      API_BASE + "/api/businesses/" + business.id + "/operating-hours",
    )
      .then(function (r) {
        return r.json();
      })
      ...
      .catch(function () {
        showMessage("error", "Failed to load operating hours");
      });
  }
  ...
  function addBlockedDate() {
    ...
    fetch(API_BASE + "/api/businesses/" + business.id + "/block-date", {
      ...
      body: JSON.stringify({
        date: newBlockedDate,
        reason: newBlockedReason || "Blocked",
      }),
    })
      .then(function (r) {
        return r.json();
  ...
  function removeBlockedDate(date) {
    ...
    fetch(API_BASE + "/api/businesses/" + business.id + "/block-date/" + date, {
      ...
      .then(function (data) {
        if (data.success) {
          setBlockedDates(
            blockedDates.filter(function (d) {
              return d.date !== date;
            }),
          );
  ...
  function generateVerificationCode() {
    ...
    fetch(
      API_BASE + "/api/businesses/" + business.id + "/generate-verification",
      {
        method: "POST",
      },
    )
      ...
      .then(function (data) {
        if (data.success) {
          ...
          showMessage(
            "success",
            "Verification code generated. Add this TXT record to your DNS.",
          );
    ...
    fetch(API_BASE + "/api/businesses/" + business.id + "/check-verification", {
      ...
      .then(function (data) {
        if (data.success) {
          ...
          showMessage(
            "error",
            data.error || "Verification failed. Make sure DNS is configured.",
          );
      })
      .catch(function () {
        showMessage("error", "Something went wrong");
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        "button",
        { onClick: onBack, style: backButtonStyle },
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            "div",
            { style: hoursContainerStyle },
            operatingHours.map(function (hour, idx) {
              return React.createElement(
                "div",
                { key: idx, style: hourRowStyle },
                React.createElement(
                  "div",
                  {
                    style: {
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(Save, { size: isDesktop ? 16 : 14 }),
                  ...
                  return React.createElement(
                    ...
                    React.createElement(
                      ...
                      React.createElement(
                        ...
                        new Date(date.date).toLocaleDateString("en-NG", {
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                        }),
